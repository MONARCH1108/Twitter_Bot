...
class CNNCrawler:
    def __init__(self, model_name="llama3.2:latest"):
        ...
        # CNN URL patterns for better article discovery
        self.url_patterns = {
            'politics': [
            ...
            'health': [
                ...
                'https://www.cnn.com/health/wellness'
            ],
            'tech': [
                'https://www.cnn.com/business/tech',
            ...
            'entertainment': [
                ...
                'https://www.cnn.com/entertainment/tv'
            ]
        }
    ...
    def discover_article_urls(self, base_urls, max_per_category=5):
        ...
        def fetch_single_category(url):
            try:
                ...
                # CNN-specific selectors for article links
                link_selectors = [
                    'a[data-link-type="article"]',
                    'h3.cd__headline a',
                    '.container__link',
                    '.cd__content a[href*="/2024/"], .cd__content a[href*="/2025/"]',
                    'a[href*="/politics/"], a[href*="/world/"], a[href*="/business/"]',
                    'a[href*="/health/"], a[href*="/entertainment/"]',
                    '.card-media a',
                    '.list-content a',
    ...
    def is_valid_article_url(self, url):
        ...
        if not url or not ('cnn.com' in url):
            return False
        
        exclude_patterns = [
            '/live-news/', '/videos/', '/interactive/', '/specials/',
            '/audio/', '/newsletters/', '/profiles/', '/about/',
            '/privacy/', '/terms/', '/contact/', '/careers/',
            '.json', '.xml', '.css', '.js', '.png', '.jpg',
            '#', 'mailto:', 'tel:', '/gallery/', '/photos/'
        ]
        ...
        # CNN articles typically have year in URL path
        valid_patterns = [
            '/2024/', '/2025/', '/politics/', '/world/', 
            '/business/', '/health/', '/entertainment/', '/tech/'
        ]
    ...
    def extract_content_direct(self, urls):
        ...
        def extract_single_article(url):
            try:
                ...
                # Extract title - CNN specific selectors
                title_selectors = [
                    'h1.headline__text',
                ...
                # Extract content - CNN specific selectors
                content_selectors = [
                    '.article__content p',
                    '.zn-body__paragraph',
    ...
    def analyze_with_ollama_fast(self, content, category):
        ...
        prompt = f"""Analyze this CNN {category} article briefly. Return only JSON:
...
{{"headline":"Main headline","summary":"Brief summary","key_topics":["topic1","topic2"],"sentiment":"positive/negative/neutral","urgency":"high/medium/low"}}"""
    ...
    def save_data(self, data, filename='cnn_crawl_data.json'):
...
if __name__ == "__main__":
    print("📰 CNN News Crawler - Getting the latest from CNN!")
    
    crawler = CNNCrawler(model_name="llama3.2:latest")
    ...
    if data and data.get('summary', {}).get('total_articles', 0) > 0:
        ...
        # Save data
        crawler.save_data(data, 'cnn_crawl_data.json')
        
        print(f"\n✅ CNN crawling completed successfully!")
        print(f"💾 File saved: cnn_crawl_data.json")
        
    else:
        print("❌ No articles were successfully extracted.")
        print("Possible issues:")
        print("- Network connectivity")
        print("- CNN website blocking requests")
